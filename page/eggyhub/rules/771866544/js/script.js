...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // 获取所有重要规则项（新增）
    const importantItems = document.querySelectorAll('.rule-item-important');

    // 添加普通规则项点击交互效果（已增加互斥逻辑）
    ruleItems.forEach(item => {
        item.addEventListener('click', function() {
            ...
            if (this.classList.contains('active')) {
                ...
                // 移除所有重要规则项的高亮（互斥）
                importantItems.forEach(impItem => {
                    impItem.classList.remove('active');
                    impItem.style.backgroundColor = '';
                });
        ...
        impItem.addEventListener('click', function() {
            ...
            if (this.classList.contains('active')) {
                ...
                ruleItems.forEach(ruleItem => {
                    ...
                    ruleItem.style.backgroundColor = '';
                });
                // 移除其他重要规则项的高亮（唯一原则）
                importantItems.forEach(otherImp => {
                    if (otherImp !== this) {
                        otherImp.classList.remove('active');
                        otherImp.style.backgroundColor = '';
                    }
                });

                // 高亮当前重要规则项
                this.classList.add('active');
                this.style.backgroundColor = '#fedddd';  // 重要规则项专用背景色
            }
        });
    });
